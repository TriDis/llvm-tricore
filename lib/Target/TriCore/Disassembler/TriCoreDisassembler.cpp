...
static DecodeStatus DecodeRCLInstruction(MCInst &Inst,
                                               unsigned Insn,
                                               uint64_t Address,
                                               const void *Decoder);
...
static DecodeStatus DecodeOperandRegister(MCInst &Inst,
                                             unsigned RegNo,
                                             uint64_t Address,
                                             const void *Decoder)
{
  // Decode Data Register
  DecodeStatus status = DecodeDataRegsRegisterClass(Inst, RegNo, Address, Decoder);
...
DecodeSBRInstruction(MCInst &Inst, unsigned Insn, uint64_t Address,
  ...
  // Decode s2.
  DecodeStatus status = DecodeOperandRegister(Inst, s2, Address, Decoder);
  if (status != MCDisassembler::Success)
    return status;
...
DecodeSLROInstruction(MCInst &Inst, unsigned Insn, uint64_t Address,
  ...
  // Decode d.
  DecodeStatus status = DecodeOperandRegister(Inst, d, Address, Decoder);
  if (status != MCDisassembler::Success)
    return status;
...
DecodeSRInstruction(MCInst &Inst, unsigned Insn, uint64_t Address,
  ...
  // Decode s1/d.
  DecodeStatus status = DecodeOperandRegister(Inst, s1_d, Address, Decoder);
  if (status != MCDisassembler::Success)
    return status;
...
DecodeSSROInstruction(MCInst &Inst, unsigned Insn, uint64_t Address,
  ...
  // Decode s1.
  DecodeStatus status = DecodeOperandRegister(Inst, s1, Address, Decoder);
  if (status != MCDisassembler::Success)
    return status;
...
DecodeBOInstruction(MCInst &Inst, unsigned Insn, uint64_t Address,
  ...
  unsigned s2 = fieldFromInstruction(Insn, 28, 4);
  unsigned s1_d = fieldFromInstruction(Insn, 22, 4);
...
static DecodeStatus
DecodeRCLInstruction(MCInst &Inst, unsigned Insn, uint64_t Address,
                    const void *Decoder) {
